...
export default function BettingHistoryPage() {
  ...
  const bettingHistory = [
    ...
    },
    {
      id: 2,
      date: "2024-11-12",
      prediction: "MON will outperform ETH in Q4",
      amount: 25,
      odds: "2.2x",
      result: "pending",
      payout: 55,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <div className="glass-card p-4 text-center">
          ...
          <div className="text-2xl font-bold text-secondary">+{stats.totalProfit} MON</div>
        </div>
        <div className="glass-card p-4 text-center">
          <div className="text-sm text-text-muted">Biggest Win</div>
          <div className="text-2xl font-bold text-accent">{stats.biggestWin} MON</div>
        </div>
      </div>
            ...
            <div>
              ...
              <select className="w-full rounded-md border border-border-input bg-bg-card px-3 py-1.5 text-sm focus:border-primary focus:outline-none">
                <option value="all">Any Amount</option>
                            <option value="small">Small (&lt; 20 MON)</option>
            <option value="medium">Medium (20-50 MON)</option>
            <option value="large">Large (&gt; 50 MON)</option>
              </select>
            </div>
          </div>
        )}
              ...
              {sortedBets.length > 0 ? (
                sortedBets.map((bet) => (
                  <tr key={bet.id} className="border-b border-border-card hover:bg-bg-card">
                    ...
                    <td className="p-4 font-medium">{bet.amount} MON</td>
                    ...
                    <td className={`p-4 font-medium ${
                      ...
                      {bet.profit > 0 ? `+${bet.profit}` : bet.profit} MON
                    </td>
                  </tr>
                ))
              ) : (
